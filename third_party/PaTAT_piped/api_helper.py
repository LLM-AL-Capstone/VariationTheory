...
from .linear_network import patterns_against_examples, train_linear_mode
from .penality_based_threaded import Synthesizer
from .helpers import dict_hash
from .helpers import get_patterns
from .helpers import get_similarity_dict
from .helpers import expand_working_list
from .helpers import NN_cluster
from .helpers import NN_classificaion
from .helpers import NN_multi_classificaion
from .helpers import pattern_clusters
from .cache_helper import RepeatedTimer
...
class APIHelper:
    ...
    def __init__(self, user="test"):
        ...
        path = f"data/{self.theme}.csv"
        if os.path.exists(path):
            self.data = pd.read_csv(path, delimiter=",")
        else:
            self.data = None

        # Don’t build similarity until data is present
        self.words_dict, self.similarity_dict = {}, {}
        if self.data is not None and "example" in self.data.columns:
            # include your helper’s full signature if it expects extra args
            self.words_dict, self.similarity_dict = get_similarity_dict(
                self.data["example"].values,
                soft_threshold=self.soft_threshold,
                soft_topk_on=getattr(self, "soft_topk_on", False),
                soft_topk=getattr(self, "topk", 10),
                file_name=self.theme,
            )
    ...
    def set_data(self, df, theme=None, rebuild_similarity=True):
        """Inject a dataset (expects columns: id, example, Label)."""
        if theme is not None:
            self.theme = theme
            self.data = df
        if rebuild_similarity and "example" in df.columns:
            self.words_dict, self.similarity_dict = get_similarity_dict(
                df["example"].values,
                soft_threshold=self.soft_threshold,
                soft_topk_on=getattr(self, "soft_topk_on", False),
    ...
    def stash_stuff(self):
        ...
        cache = {
            ...
            "synthesizer_collector":{theme:{"results":self.synthesizer_collector[theme].results, 
                            ...
                            "df_tracker": ("[]" 
                                           if self.synthesizer_collector[theme].df_tracker is None
                                            else self.synthesizer_collector[theme].df_tracker
                                                .reset_index(drop=True)
                                                .applymap(lambda x: x if isinstance(x, (str, int, float, bool, type(None))) else str(x))
                                                .to_json(orient="records")
                                                )} for theme in self.synthesizer_collector.keys()}
        }
